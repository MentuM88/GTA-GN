...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ Pour arreter le Golf.")
			else
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ pour débuter le Golf (€100).")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Appuyer sur E pour swinger, A-D pour tourner, Y pour changer de Club.")
				end
			end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Aller à votre balle, appuyez sur ~g~E~s~ si vous êtes coincé.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Vous avez reussi!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Vous avez pris trop de tirs..")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end

	Citizen.Trace("On dirait que la balle a aterri dans : " .. GetCollisionNormalOfLastHitForEntity(mygolfball) )
end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Appuyez sur ~g~E~s~ pour lacher ici.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Appuyez sur ~g~E~s~ pour lacher - ~r~ trop loin de la balle, ou trop proche du trou.")
		end
	end
end
