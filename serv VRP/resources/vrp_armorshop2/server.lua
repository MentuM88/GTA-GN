...
vRPclient = Tunnel.getInterface("vRP","vrp_armorshop2")
ASclient = Tunnel.getInterface("vrp_armorshop2","vrp_armorshop2")
...
local cfg = module("vrp_armorshop2", "cfg/gunshops")
...
--[[      local user_id = vRP.getUserId({player})
      ...
      if weapon == "HEAVYARMOR" then-- get player weapons to not rebuy the body
        -- payment
        if user_id ~= nil and vRP.tryFullPayment({user_id,price}) then
          ASclient.setArmour(player,{200,true})
          vRPclient.notify(player,{lang.money.paid({price})})
        else
          vRPclient.notify(player,{lang.money.not_enough()})
      end
