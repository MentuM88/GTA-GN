--[[
#####################################################################################################
           - Menu Animations Avec Possibilité de save sa position By Super.Cool.Ninja
#####################################################################################################
--]]
local options = {
    x = 0.900,
    ...
    scale = 0.4,
    font = 2,
    menu_title = "Ninja Menu", --Modifier le Nom du Menu Ici !
...
function PersonnalMenu()
	...
	Menu.addButton("Sauvegarder sa position", "saveposition", nil)
end
------------------------------------------------------------------------------------------------------------------------
...
function saveposition() --Function qui permet de Save ma position
    --Récupération de la position x, y, z du joueur
    posX, posY, posZ = table.unpack(GetEntityCoords(GetPlayerPed(-1), true)) --Position
    --Récupération de l'azimut du joueur
    local LastPosH = GetEntityHeading(GetPlayerPed(-1))
    --Envoi des données vers le serveur
    TriggerServerEvent("esx:requestthePosition", posX, posY, posZ, LastPosH) --Mon Event qui me permet de save ma position
        --Affichage d'un message confirmant la sauvegarde
        Notify("~g~Position Sauvegardée.") --Notification qui permet de valider ma Position
        PersonnalMenu()
end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsControlJustPressed(1, 160) then --Modifier L'ouverture du Menu Ici !
            PersonnalMenu()
			-- Menu to draw
