...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Conduire à la ~g~destination~w~ marquée.", 10000)
            text1 = true
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "Ramenez le camion au dépot pour prendre une autre mission."..currentMission[4], 15000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Arrivé. Détachez votre ~o~remorque~w~ avec ~r~H~w~", 15000)
        end
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "Vous voulez être chauffeur? Pressez ~r~N+~w~ pour commencer.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("Vous êtes un chauffeur maintenant.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("Choisissez votre remorque.", 0.575, 0.375, 0.15, 0.30 )
