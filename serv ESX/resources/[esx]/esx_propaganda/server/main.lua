		...
		if #topay > 0 then 
			...
			'DELETE FROM paychecks WHERE receiver = @identifier', 
			{
			  ['@identifier'] = xPlayer.identifier
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, "~g~ Vous avez un salaire " .. amount)
				xPlayer.addMoney(amount)
			end
			)
			
		else
			TriggerClientEvent('esx:showNotification', xPlayer.source, "~r~ Personne n'a aimé vos articles :(")
		end
	end
	)
...
AddEventHandler('esx_propaganda:postArticle', function(dataTemp)
	...
	{},
	function(rows2)
		TriggerClientEvent('esx:showNotification', xPlayer.source, "~w~ Vieux articles ~ r ~ enlevés ~ w ~!")
	end
	)
