...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    {label = _U('armurier_wear'), value = 'armurier_wear'}
  }
  ...
  ESX.UI.Menu.CloseAll()
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'armurier_wear' then
				isInService = true
          ...
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_armurierjob:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_armurierjob:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_armurierjob:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                  TriggerServerEvent('esx_armurierjob:OutVehicle', GetPlayerServerId(player))
              end
                  ...
                  function (data2, menu)
                    ...
                    if amount == nil then
                      ...
                      if closestPlayer == -1 or closestDistance > 5.0 then
                        ...
                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_armurier', 'Banque', amount)
                      end
                    end
                  end,
                  function (data2, menu)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_armurierjob:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_armurierjob:getVehicleInfos', function(infos)
...
AddEventHandler('esx_armurierjob:hasEnteredMarker', function(station, part, partNum)
  ...
  if part == 'HelicopterSpawner' then
    ...
    local helicopters = Config.ArmurierStations[station].Helicopters
...
RegisterNetEvent('esx_armurierjob:handcuff')
AddEventHandler('esx_armurierjob:handcuff', function()
...
RegisterNetEvent('esx_armurierjob:drag')
AddEventHandler('esx_armurierjob:drag', function(cop)
